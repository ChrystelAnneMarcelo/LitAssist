...
async def create_chat(body: ChatSessionCreate, userId: str = "guest", db: AsyncIOMotorDatabase = Depends(get_db)):
...
async def get_chat(chat_id: str, db: AsyncIOMotorDatabase = Depends(get_db)):
    doc = await db.chat_sessions.find_one({"id": chat_id}, {"_id": 0})
    if not doc:
...
async def delete_chat(chat_id: str, db: AsyncIOMotorDatabase = Depends(get_db)):
    res = await db.chat_sessions.delete_one({"id": chat_id})
    if res.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Chat session not found")


@router.post("/{chat_id}/messages", response_model=ChatMessage, status_code=201)
async def add_message(chat_id: str, body: ChatMessageCreate, db: AsyncIOMotorDatabase = Depends(get_db)):
    message = ChatMessage(id=str(uuid.uuid4()), **body.model_dump())
    res = await db.chat_sessions.update_one({"id": chat_id}, {"$push": {"messages": message.model_dump()}})
